...
```python API_KEY = "<<INTRODUCE YOUR API_KEY HERE>>"
...
``` python def minimal_etl_to_forecaster(df):
...
```python out['_main']['preprocessing_summary']['summary_main']['dataset_summary']['number_of_series']
```
```python out['_main']['preprocessing_summary']['summary_main']['transformers_info']['white-noise-test']['noisy'][:8]
```
```python out['_main']['preprocessing_summary']['summary_main']['transformers_info']['stationarity-test']['non-stationary'][:8]
```
...
```python out['_main']['training_summary']['time_to_fit_seconds']
```
```python out['_main']['training_summary']['fit_results']
```
```python forecast = pd.DataFrame(out['forecast'])
